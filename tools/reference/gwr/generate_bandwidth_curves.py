#!/usr/bin/env python3
"""Generate controlled adaptive GWR bandwidth criterion curves.

The same integer-neighbour candidates are evaluated by PyGWRx and mgwr.  This
...
PYGWRX_OUTPUT = DATA_DIR / "pygwrx_bandwidth_curve.json"
MGWR_OUTPUT = DATA_DIR / "mgwr_bandwidth_curve.json"

# For this fixture n=40 and the fitted design has three columns (intercept+x1+x2).
# PyGWRx's automatic adaptive range is therefore 4..40, and all implementations
# are evaluated explicitly on that same candidate set.
CANDIDATES = tuple(range(4, 41))
...
def _pygwrx_cv_sse(
    ...
    for i, dists in enumerate(distances):
        ...
        # Match CrossValidationSelector exactly: strict leave-one-out at the focal row.
        weights[i] = 0.0
...
def _mgwr_curve(
    ...
    for k in CANDIDATES:
        try:
            ...
            # mgwr's get_CV returns mean squared leave-one-out error; multiply by n
            # so cv_sse uses the same scale as PyGWRx and GWmodel.
...
def main() -> None:
    ...
    X_frame = frame[["x1", "x2"]]
    y_series = frame["response"]
    coords_frame = frame[["x", "ycoord"]]
