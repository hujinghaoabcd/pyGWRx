...
- C2 — GWR engine numerical lock：**DONE**，PR #62，merge SHA `0fd763dc8df7a4185f532e35b48c320d8a0e5215`。在 C1 private engine 边界新增 blocking external-reference numerical locks，直接使用既有冻结的 mgwr 2.2.1 artifacts 验证 5 类 calibration/inference 场景与 independent-location prediction；reference suite 从 50 增至 56 tests，并继续覆盖 deep hat/influence/Cook's D、rank-deficient behavior、streaming、bandwidth provenance 与 failed-refit atomicity。Quality、coverage、minimum dependencies、build/install、Ubuntu/macOS/Windows × Python 3.11–3.14、Documentation 与 Security/SBOM 全部通过；未修改 runtime、统计公式、reference fixture、tolerance、search semantics 或 public API。
- C3 — freeze GWR protected subclass surface：**DONE**，PR #64，merge SHA `8f32a8246806d15129a7f48a9fcfb67123f0aa2c`。新增 machine-readable `architecture_contracts/gwr_protected_surface.json` 与 AST/runtime 双重 contract tests，把 C3 时点 GWR 自身声明的 12 个单下划线 protected 方法冻结为最大允许集合：后续阶段可删除旧兼容 hook，但不得新增替代性的 GWR protected implementation method；新增实现辅助逻辑必须进入 private engine 或模型自有模块。`gwr.py` runtime 未修改，RGWR/LCRGWR/GWGLM concrete inheritance cleanup 继续严格留给 D1/D2/D3。56 个 blocking references、Quality、coverage、minimum dependencies、build/install、Ubuntu/macOS/Windows × Python 3.11–3.14、Documentation 与 Security/SBOM 全部通过；未修改 estimator API、统计公式、数值结果、search semantics、reference fixture 或 tolerance。
- 下一项固定为：**D1 — RGWR remove concrete GWR inheritance**。

A1/A2/A3/A4/A5、B1/B2/B3/B4/B5/B6/B7/B8、C1、C2 与 C3 共同构成进入 D1 的安全冻结：Public API / capability、failed-refit atomicity、迁移风险、性能/内存现状、用户可观察契约、private capability spine、canonical validation/distance/time/weights/WLS/search-domain/Gaussian-diagnostics 边界、standard-GWR private engine ownership、56 个 blocking reference tests 的 numerical lock，以及 GWR protected implementation surface 的 non-growing contract 必须同时守住。A4 的 timing/RSS 仅作为 observational baseline，不设置 noisy wall-time CI gate；live structural memory guards 继续阻止 streamed GWR 意外引入 retained `n × n` buffer，并保留 `compute_hat_matrix=True` 的显式 dense S 行为。D1 只允许移除 RGWR 对 concrete `GWR` estimator 的继承依赖并改为 composition/capability reuse，同时保持 RGWR public API、fitted-state atomicity、robust weighting/statistical behavior、prediction/inference/result semantics 与 numerical evidence；不得修改 reference fixture/tolerance，不得顺手处理 LCRGWR/GWGLM、base rewrite 或无关性能算法。
