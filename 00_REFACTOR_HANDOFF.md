...
- 下一项固定为：**B7 — generic search domains**。

A1/A2/A3/A4/A5 与 B1/B2/B3/B4/B5/B6 共同构成进入后续 Phase B 的安全冻结：Public API / capability、failed-refit atomicity、迁移风险、性能/内存现状、用户可观察契约、private capability spine，以及 canonical validation/distance/time/weights/WLS 边界必须同时守住；标准 GWR 的独立 reference evidence 继续作为 blocking numerical gate。A4 的 timing/RSS 仅作为 observational baseline，不设置 noisy wall-time CI gate；live structural memory guards 用于阻止 streamed GWR 与 large-n ScalableGWR 意外引入 retained `n × n` buffer，并保留 dense-hat GWR 的显式 dense 行为。B7 只允许抽取 generic search-domain / optimization infrastructure，objective 必须继续 model-owned；不得改变 bandwidth/selector objective、搜索轨迹或模型统计定义，不得提前迁移 B8 diagnostics，不得 Base 重写、concrete inheritance 移除、性能优化或 reference/tolerance 调整。
